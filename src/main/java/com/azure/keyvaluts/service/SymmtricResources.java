...
import com.azure.keyvaluts.common.ApplicationProperties;
import com.azure.keyvaluts.common.DBConstants;
import com.azure.keyvaluts.common.Encryption;
import com.azure.keyvaluts.common.LogFormatter;
...
public class SymmtricResources {
  ...
  private static final Logger LOG = LogManager.getLogger(SymmtricResources.class);
  ...
  public ResponseEntity saveEncryptedData(UserRequest userRequest) {
    try {
      ...
      String iv = applicationProperties.getKeys();
      String keys = applicationProperties.getKeys();
      LOG.info(LogFormatter.instance()
          .message("Cleaning up ").data("password", keys).format());
      ...
      for (DataValues user : userRequest.getDataValues()) { // O(n)
        ...
        if (userRequest.getEncryptdKey().contains(DBConstants.FieldsName.FIRSTNAME.value())) {
          ...
          String encyptedValue = encryption.encryptData(rawValue, keys.getBytes(), iv.getBytes(),
              encodealgo, encryptalgo);
    ...
    try {
      ...
      for (DataValues user : userRequest.getDataValues()) {
        ...
        if (userRequest.getEncryptdKey().contains(DBConstants.FieldsName.FIRSTNAME.value())) {
          ...
          String rawValue =
              encryption.decryptData(encyptedValue, keys.getBytes(), keys, encodealgo, encryptalgo);
